...
def replace_icon(exe_path, icon_path, output_path):
    command = [
        "ResourceHacker.exe",  # Resource Hacker executable path
        "-open", exe_path,  # Path to the EXE file
        "-save", output_path,  # Path to save the modified EXE file (overwrite)
        "-action", "addoverwrite",  # Add and overwrite resources
        "-res", icon_path,  # Path to the new icon file
        "-mask", "ICONGROUP,DEMO",  # Specify the icon resource to replace
    ]
    # Run Resource Hacker command
    subprocess.run(command)

def replace_signature(exe_path, signature_exe_path, output_path):
    if signature_exe_path:
        signature = copyCert(signature_exe_path)
        writeCert(signature, exe_path, output_path)
    else:
        shutil.copyfile(exe_path, output_path)

def replace_signature_and_icon(inputfile, signature, icon, outputfile):
    replace_icon(inputfile,icon,outputfile)
    dir_name = os.path.dirname(outputfile)
    demo_path = os.path.join(dir_name, uuid.uuid4().hex[:10]+'.exe')
    os.rename(outputfile, demo_path)
    replace_signature(demo_path, signature, outputfile)
    os.remove(demo_path)